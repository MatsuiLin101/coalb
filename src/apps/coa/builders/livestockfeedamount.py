...
class LivestockFeedAmountBuilder(object):
    """
    建立 畜禽在養數量api介面 LivestockFeedAmountApiView 使用的選項

from apps.coa.builders.livestockfeedamount import *
...
builder.build()
    ...
    def __init__(self):
        ...
        self.text_group2 = '家禽飼養隻數：縣市別×家禽別'
        ...
        for option in options:
            ...
            if parent is not None:
                if level - parent.level != 1:
                    parent = LivestockFeedAmount.objects.filter(main_class=text_group, sub_class='city', level=level-1).last()
    ...
    def build_product(self, text_group):
        ...
        for option in options:
            ...
            if parent is not None:
                if level - parent.level != 1:
                    parent = LivestockFeedAmount.objects.filter(main_class=text_group, sub_class='product', level=level-1).last()

...
class LivestockSlaughterBuilder(object):
    """
    建立 畜禽供應屠宰量api介面 LivestockSlaughterApiView 使用的選項

from apps.coa.builders.livestockslaughter import *
...
builder.build()
    ...
    def build_city(self, text_group):
        ...
        options = group_city.find_elements(By.TAG_NAME, 'option')
        parent = None
        for option in options:
            ...
            if parent is not None:
                if level - parent.level != 1:
                    parent = LivestockSlaughter.objects.filter(main_class=text_group, sub_class='city', level=level-1).last()
            ...
            obj = LivestockSlaughter.objects.create(
                parent = parent,
    ...
    def build_product(self, text_group):
        ...
        options = group_product.find_elements(By.TAG_NAME, 'option')
        parent = None
        for option in options:
            value = option.get_attribute('value')
            name = option.text
            ...
            if parent is not None:
                if level - parent.level != 1:
                    parent = LivestockSlaughter.objects.filter(main_class=text_group, sub_class='product', level=level-1).last()

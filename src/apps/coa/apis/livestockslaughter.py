...
class LivestockSlaughterApiView(BasicApiView):
    ...
    def __init__(self, params):
        ...
        self.text_group2 = '家禽供應屠宰隻數：縣市別×家禽別'
        ...
        if len(params) not in [3, 4]:
            raise CustomError('供應屠宰量的指令為「屠宰 畜禽 年份」或「屠宰 畜禽 年份 縣市」，例如：\n「屠宰 豬 108」\n「屠宰 鴨 109 彰化」')
    ...
    def get_group(self):
        ...
        # 是否有完全符合name的物件
        try:
            self.obj_product = LivestockSlaughter.objects.get(name=self.product, sub_class='product')
            return
        ...
        # 是否有部分符合name的物件
        try:
            self.obj_product = LivestockSlaughter.objects.get(name__icontains=self.product, sub_class='product')
            return
        except Exception as e:
            pass
        ...
        try:
            self.obj_product = LivestockSlaughter.objects.get(search_name=self.product, sub_class='product')
            return
        except Exception as e:
            pass

        # 可能有多種結果，請使用者改用詳細關鍵字
        qs = LivestockSlaughter.objects.filter(name__icontains=self.product, sub_class='product')
        if qs.count() > 0:
    ...
    def get_city(self):
        # 是否有完全符合name的物件
        try:
            self.obj_city = LivestockSlaughter.objects.get(name=self.city, main_class=self.obj_product.main_class, sub_class='city')
            return
        except Exception as e:
            pass
        ...
        # 是否有部分符合name的物件
        try:
            self.obj_city = LivestockSlaughter.objects.get(name__icontains=self.city, main_class=self.obj_product.main_class, sub_class='city')
            return
        except Exception as e:
            pass
        ...
        # 是否有完全符合search_name的物件
        try:
            self.obj_city = LivestockSlaughter.objects.get(search_name=self.city, main_class=self.obj_product.main_class, sub_class='city')
            return
        except Exception as e:
            pass

        # 可能有多種結果，請使用者改用詳細關鍵字
        qs = LivestockSlaughter.objects.filter(name__icontains=self.city, main_class=self.obj_product.main_class, sub_class='city')
        if qs.count() > 0:

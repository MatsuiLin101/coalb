...
class LivestockFeedlotApiView(ApiView):
    ...
    def api(self):
        try:
            ...
            if not self.message:
                self.message = f"搜尋「場數 {self.query_date} {self.product}」發生錯誤"
        if self.driver:
            self.driver.close()
        return self.message
    ...
    def get_data(self):
        ...
        self.get_result()
        if not self.message:
            self.message = f"搜尋「場數 {self.year} {self.product}_city_1」的結果為：\n" + f"{self.year}年 {self.obj_product.name}_city_2 飼養場數：{self.result}(場)"
            if self.city is not None:

...
class WelfareApiView(BasicApiView):
    ...
    def choose_api(self):
        ...
        elif self.command in ['獎學金', '助學金', '獎助學金']:
...
class Insurance(WelfareApiView):
    ...
    def __init__(self, params):
        ...
        if not 2 <= len(params) <= 3:
            raise CustomError('農保的指令為「農保 年份」或「農保 年份/月份」，例如：\n「農保 108」\n「農保 108/3」\n\n也可加上縣市「農保 縣市 年份」或「農保 縣市 年份/月份」，例如：\n「農保 高雄 109」\n「農保 高雄 109/3」')
        self.command = params[0]
        if len(params) == 3:
...
class Allowance(WelfareApiView):
    ...
    def __init__(self, params):
        ...
        if not 2 <= len(params) <= 3:
            raise CustomError('老農津貼的指令為「老農津貼 年份」或「老農津貼 年份/月份」，例如：\n「老農津貼 108」\n「老農津貼 108/3」\n\n也可加上縣市「老農津貼 縣市 年份」或「老農津貼 縣市 年份/月份」，例如：\n「老農津貼 高雄 109」\n「老農津貼 高雄 109/3」')
        self.command = params[0]
        if len(params) == 3:
            self.city = params[1].replace('台', '臺')
            self.query_date = params[2]
...
class Scholarship(WelfareApiView):
    ...
    def __init__(self, params):
        ...
        if not 2 <= len(params) <= 3:
            raise CustomError('獎助學金的指令為「獎助學金 年份」或「獎助學金 年份/月份」，例如：\n「獎助學金 108」\n「獎助學金 108/3」\n\n也可加上縣市「獎助學金 縣市 年份」或「獎助學金 縣市 年份/月份」，例如：\n「獎助學金 高雄 109」\n「獎助學金 高雄 109/3」')
        self.command = params[0]
        if len(params) == 3:
            self.city = params[1].replace('台', '臺')
            self.query_date = params[2]

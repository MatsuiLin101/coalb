...
class AbstractBuild(object):
    ...
    def build_obj(self, select_obj, category):
        for option in select_obj.options:
            name = option.text
            value = option.get_attribute("value")
            obj = self.model.objects.create(name=name, value=value, category=category)
            print(f"{obj.name} create")
...
class BuildProduceValueProduct(AbstractBuild):
    ...
    def build_produce_value_product(self, xpath, select_id, category):
        ...
        self.build_obj(select_obj, category)
        time.sleep(1)
    ...
    def build_model(self):
        ...
        self.build_produce_value_product(xpath, select_id, "rice")

        # 抓取農業產值雜糧選項
        xpath = "//option[. = '雜糧產值：縣市別×雜糧別']"
        select_id = "ctl00_cphMain_uctlInquireAdvance_dtlDimension_ctl02_lstDimension"
        self.build_produce_value_product(xpath, select_id, "grain")

        # 抓取農業產值特作選項
        xpath = "//option[. = '特作產值：縣市別×特作別']"
        select_id = "ctl00_cphMain_uctlInquireAdvance_dtlDimension_ctl02_lstDimension"
        self.build_produce_value_product(xpath, select_id, "special")

        # 抓取農業產值製糖甘蔗選項
        # xpath = "//option[. = '製糖甘蔗產值：縣市別']"
        # select_id = ""
        # self.build_produce_value_product(xpath, select_id, "sugar")
        obj = self.model.objects.create(name="甘蔗", category="sugar")
        print(f"{obj.name} create")
        ...
        obj = self.model.objects.create(name="菸草", category="tobacco")
        print(f"{obj.name} create")

        # 抓取農業產值蔬菜選項
        xpath = "//option[. = '蔬菜產值：縣市別×蔬菜別']"
        select_id = "ctl00_cphMain_uctlInquireAdvance_dtlDimension_ctl02_lstDimension"
        self.build_produce_value_product(xpath, select_id, "vegetable")

        # 抓取農業產值菇類選項
        xpath = "//option[. = '菇類產值：縣市別×菇類別']"
        select_id = "ctl00_cphMain_uctlInquireAdvance_dtlDimension_ctl02_lstDimension"
        self.build_produce_value_product(xpath, select_id, "mushroom")

        # 抓取農業產值果品選項
        xpath = "//option[. = '果品產值：縣市別×果品別']"
        select_id = "ctl00_cphMain_uctlInquireAdvance_dtlDimension_ctl02_lstDimension"
        self.build_produce_value_product(xpath, select_id, "fruits")

        # 抓取農業產值花卉選項
        xpath = "//option[. = '花卉產值：縣市別×花卉別']"
        select_id = "ctl00_cphMain_uctlInquireAdvance_dtlDimension_ctl02_lstDimension"
        self.build_produce_value_product(xpath, select_id, "flower")

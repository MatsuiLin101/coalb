...
environ.Env.read_env(os.path.join(BASE_DIR, 'envs', env.str('USE_ENV')))
...
# 設定session cookie的有效期限 default為16小時
SESSION_COOKIE_AGE = env.int('SESSION_COOKIE_AGE', 57600)  # default 16 hours in seconds
...
if env.bool('PROXY_ADMIN_EMAIL_HANDLER', default=False):
    LOGGING = {
        ...
        'handlers': {
            'mail_admins': {
                'level': 'ERROR',
                'class': 'core.log.ProxyAdminEmailHandler'
            },
        },
        'loggers': {
            'django': {
...
EMAIL_USE_TLS = env.bool('EMAIL_USE_TLS', default=587)
EMAIL_PORT = env.int('EMAIL_PORT', default=True)
